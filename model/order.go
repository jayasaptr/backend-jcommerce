...
type Order struct {
	...
	PaidAccountNumber *string    `json:"paidAccountBank,omitempty"`
}
...
func CraeteOrder(db *sql.DB, order Order, details []OrderDetail) error {
	if db == nil {
		return ErrDBNil
	}
...
func UpdateOrderByID(db *sql.DB, id string, confirm Confrim, paidAt time.Time) error {
	if db == nil {
		return ErrDBNil
	}
